...
vector<int> repeatedNumber(const vector<int> &A) {
    ...
    cout<<"Sum OF N"<<SumOfN<<endl;
    long long SumOfA=0;
    for(auto i:A)
        SumOfA+=A[i];
    cout<<"SUm OF A"<<SumOfA<<endl;
    long long SquareSumOfN=(N*(N+1)*((2*N)+1))/6;
    long long SqareSumOfA=0;
    for(auto i:A)
    {
        SqareSumOfA+=i*i;
    }
    int eq1=SumOfN-SumOfA;
    int eq2=(SquareSumOfN-SqareSumOfA)/(eq1);
    cout<<"Inside func: eq1: "<<eq1<<endl;
    cout<<"Inside func: eq2: "<<eq2<<endl;


    return {eq2+eq1,eq2-eq1};
